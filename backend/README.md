...
This repository hosts the Express web API server and Socket.io WebSocket orchestrator powering the Zenith Workspace. It manages state transitions, real-time presence rooms, transaction routing, and AI agents.

---

## 🛠️ Tech Architecture & Operations

The backend runs on Node.js using ES Modules (`import/export`) and interfaces with MongoDB Atlas for persistent storage.
...
flowchart TD
    ...
    subgraph DB ["Mongoose Data Access Model"]
        Mongoose["🗃️ MongoDB Models<br/>(User, Workspace, Card, Activity)"]
    end
    class Mongoose db;
...
### 📋 Boards & Lists (`/api/boards`, `/api/lists`)
